...
__device__ uint32_t HmqTrueTest(uint32_t *inpHash)
{
	return ((inpHash[0] & 0x18) > 0);
}

__device__ uint32_t HmqFalseTest(uint32_t *inpHash)
{
	return ((inpHash[0] & 0x18) == 0);
}

__device__ cuda_compactTestFunction_t d_HmqTrueFunction = HmqTrueTest, d_HmqFalseFunction = HmqFalseTest;

cuda_compactTestFunction_t h_HmqTrueFunction[MAX_GPUS], h_HmqFalseFunction[MAX_GPUS];

// Setup/Alloc Function
__host__
void hmq_compactTest_cpu_init(int thr_id, uint32_t threads)
{
	int dev_id = device_map[thr_id];
	cuda_get_arch(thr_id);

	cudaMemcpyFromSymbol(&h_HmqTrueFunction[thr_id], d_HmqTrueFunction, sizeof(cuda_compactTestFunction_t));
	cudaMemcpyFromSymbol(&h_HmqFalseFunction[thr_id], d_HmqFalseFunction, sizeof(cuda_compactTestFunction_t));
...
__host__
void hmq_compactTest_cpu_free(int thr_id)
{
	int dev_id = device_map[thr_id];
...
__global__
void hmq_compactTest_gpu_SCAN(uint32_t *data, const int width, uint32_t *partial_sums=NULL, cuda_compactTestFunction_t testFunc=NULL,
...
__global__
void hmq_compactTest_gpu_ADD(uint32_t *data, uint32_t *partial_sums, int len)
{
...
__global__
void hmq_compactTest_gpu_SCATTER(uint32_t *sum, uint32_t *outp, cuda_compactTestFunction_t testFunc,
	uint32_t threads=0, uint32_t startNounce=0, uint32_t *inpHashes=NULL, uint32_t *d_validNonceTable=NULL)
{
	int id = ((blockIdx.x * blockDim.x) + threadIdx.x);
...
__host__ static uint32_t hmq_compactTest_roundUpExp(uint32_t val)
{
	if(val == 0)
		return 0;
...
__host__
void hmq_compactTest_cpu_singleCompaction(int thr_id, uint32_t threads, uint32_t *nrm,uint32_t *d_nonces1,
	cuda_compactTestFunction_t function, uint32_t startNounce, uint32_t *inpHashes, uint32_t *d_validNonceTable)
{
	...
	threads = hmq_compactTest_roundUpExp(threads);
	...
	// Erster Initialscan
	hmq_compactTest_gpu_SCAN <<<thr1,blockSize>>>(
		d_tempBranch1Nonces[thr_id], 32, d_partSum[0][thr_id], function, orgThreads, startNounce, inpHashes, d_validNonceTable);	

	// weitere Scans
	if(callThrid) {
		hmq_compactTest_gpu_SCAN<<<thr2,blockSize>>>(d_partSum[0][thr_id], 32, d_partSum[1][thr_id]);
		hmq_compactTest_gpu_SCAN<<<1, thr2>>>(d_partSum[1][thr_id], (thr2>32) ? 32 : thr2);
	} else {
		hmq_compactTest_gpu_SCAN<<<thr3,blockSize2>>>(d_partSum[0][thr_id], (blockSize2>32) ? 32 : blockSize2);
	}
	...
	if(callThrid) {
		hmq_compactTest_gpu_ADD<<<thr2-1, blockSize>>>(d_partSum[0][thr_id]+blockSize, d_partSum[1][thr_id], blockSize*thr2);
	}
	hmq_compactTest_gpu_ADD<<<thr1-1, blockSize>>>(d_tempBranch1Nonces[thr_id]+blockSize, d_partSum[0][thr_id], threads);

	hmq_compactTest_gpu_SCATTER<<<thr1,blockSize,0>>>(d_tempBranch1Nonces[thr_id], d_nonces1, 
...
void hmq_filter_gpu_sm2(const uint32_t threads, const uint32_t* d_hash, uint32_t* d_branch2, uint32_t* d_NonceBranch)
{
	...
	if (thread < threads)
	{
		...
		d_NonceBranch[thread] = ((uint8_t*)psrc)[0] & 0x18;
...
__global__ __launch_bounds__(128, 8)
void hmq_merge_gpu_sm2(const uint32_t threads, uint32_t* d_hash, uint32_t* d_branch2, uint32_t* const d_NonceBranch)
{
	const uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
...
#else
__global__ void hmq_filter_gpu_sm2(const uint32_t threads, const uint32_t* d_hash, uint32_t* d_branch2, uint32_t* d_NonceBranch) {}
__global__ void hmq_merge_gpu_sm2(const uint32_t threads, uint32_t* d_hash, uint32_t* d_branch2, uint32_t* const d_NonceBranch) {}
#endif

__host__
uint32_t hmq_filter_cpu_sm2(const int thr_id, const uint32_t threads, const uint32_t *inpHashes, uint32_t* d_branch2)
{
	const uint32_t threadsperblock = 128;
	dim3 grid((threads + threadsperblock - 1) / threadsperblock);
	dim3 block(threadsperblock);
	// extract algo permution hashes to a second branch buffer
	hmq_filter_gpu_sm2 <<<grid, block>>> (threads, inpHashes, d_branch2, d_tempBranch1Nonces[thr_id]);
	return threads;
}

__host__
void hmq_merge_cpu_sm2(const int thr_id, const uint32_t threads, uint32_t *outpHashes, uint32_t* d_branch2)
{
	const uint32_t threadsperblock = 128;
	dim3 grid((threads + threadsperblock - 1) / threadsperblock);
	dim3 block(threadsperblock);
	// put back second branch hashes to the common buffer d_hash
	hmq_merge_gpu_sm2 <<<grid, block>>> (threads, outpHashes, d_branch2, d_tempBranch1Nonces[thr_id]);
}

////// ACHTUNG: Diese funktion geht aktuell nur mit threads > 65536 (Am besten 256 * 1024 oder 256*2048)
__host__
void hmq_compactTest_cpu_dualCompaction(int thr_id, uint32_t threads, uint32_t *nrm, uint32_t *d_nonces1,
	 uint32_t *d_nonces2, uint32_t startNounce, uint32_t *inpHashes, uint32_t *d_validNonceTable)
{
	hmq_compactTest_cpu_singleCompaction(thr_id, threads, &nrm[0], d_nonces1, h_HmqTrueFunction[thr_id], startNounce, inpHashes, d_validNonceTable);
	hmq_compactTest_cpu_singleCompaction(thr_id, threads, &nrm[1], d_nonces2, h_HmqFalseFunction[thr_id], startNounce, inpHashes, d_validNonceTable);
}

__host__
void hmq_compactTest_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *inpHashes,
	uint32_t *d_validNonceTable, uint32_t *d_nonces1, uint32_t *nrm1, uint32_t *d_nonces2, uint32_t *nrm2, int order)
{
	...
	hmq_compactTest_cpu_dualCompaction(thr_id, threads,
...
__host__
void hmq_compactTest_single_false_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *inpHashes,
	uint32_t *d_validNonceTable, uint32_t *d_nonces1, uint32_t *nrm1, int order)
{
	// Wenn validNonceTable genutzt wird, dann werden auch nur die Nonces betrachtet, die dort enthalten sind
	// "threads" ist in diesem Fall auf die Länge dieses Array's zu setzen!

	hmq_compactTest_cpu_singleCompaction(thr_id, threads, h_numValid[thr_id], d_nonces1, h_HmqFalseFunction[thr_id], startNounce, inpHashes, d_validNonceTable);
